...
class Universe:
	...
	def __draw_space__(self):
		...
		for world in self.__worlds__:
			world.draw(self.__display__)
	...
	def __handle_events__(self):
		for e in pyg.event.get():
			...
			if e.type == pyg.QUIT or (e.type == pyg.KEYDOWN and e.key == pyg.K_q):
			...
			elif e.type == pyg.WINDOWHIDDEN or (e.type == pyg.KEYDOWN and e.key == pyg.K_ESCAPE):
				...
				self.__worlds__.append(World(universe_dimensions))
